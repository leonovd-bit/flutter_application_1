...
class CircleOfHealthPageV1 extends StatefulWidget {
  const CircleOfHealthPageV1({super.key});

  @override
  State<CircleOfHealthPageV1> createState() => _CircleOfHealthPageV1State();
}

class _CircleOfHealthPageV1State extends State<CircleOfHealthPageV1> {
  final _auth = FirebaseAuth.instance;
  late final PageController _pageController;

  // Keep 14 days for easier scrolling; index 0 = today; reverse: true lets swiping left go to previous.
  static const int _daysCount = 14;
  String? _planName;
  ...
  void initState() {
    super.initState();
    _pageController = PageController(initialPage: 0, viewportFraction: 1.0, keepPage: true);
    _loadHeader();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Circle of Health', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold)),
        ...
        children: [
          ...
          Expanded(
            child: PageView.builder(
              ...
              reverse: true, // left swipe -> previous day
...
class _DailyPageState extends State<_DailyPage> {
  ...
  double _pct(double val, double target) => target > 0 ? (val / target).clamp(0.0, 1.0) : 0.0;
    ...
    // Deterministic demo data: vary by weekday
    final w = widget.date.weekday; // 1..7
    double pct(int base) => (base + (w * 7) % 30) / 100.0; // 0.0..1.0
    return _DailyNutrition(
      calories: pct(55),
      protein: pct(60),
      fats: pct(50),
